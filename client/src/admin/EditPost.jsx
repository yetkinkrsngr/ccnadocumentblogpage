  ...
  return (
    <div className="max-w-3xl">
      ...
      <form onSubmit={save} className="bg-white rounded-xl shadow-soft p-6 space-y-4">
        <div>
          <label htmlFor="title" className="block text-sm text-gray-600 mb-1">Başlık</label>
          <input id="title" className="w-full border rounded-lg px-3 py-2" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required />
        </div>
        <div>
          <label htmlFor="summary" className="block text-sm text-gray-600 mb-1">Özet</label>
          <textarea id="summary" className="w-full border rounded-lg px-3 py-2" rows={2} value={form.summary} onChange={e => setForm(f => ({ ...f, summary: e.target.value }))} required />
        </div>
        <div>
          <label htmlFor="category" className="block text-sm text-gray-600 mb-1">Kategori</label>
          <select id="category" className="w-full border rounded-lg px-3 py-2" value={form.categoryId} onChange={e => setForm(f => ({ ...f, categoryId: e.target.value }))} required>
            <option value="">Seçiniz</option>
            {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="author" className="block text-sm text-gray-600 mb-1">Yazar</label>
          <input id="author" className="w-full border rounded-lg px-3 py-2" value={form.author} onChange={e => setForm(f => ({ ...f, author: e.target.value }))} />
        </div>
        <div>
          <div className="flex items-center justify-between">
            <label htmlFor="content" className="block text-sm text-gray-600 mb-1">İçerik (Markdown)</label>
            <button type="button" onClick={() => setMediaOpen(true)} className="text-sm px-3 py-1 rounded border">Medya Ekle</button>
          </div>
          <textarea id="content" ref={contentRef} className="w-full border rounded-lg px-3 py-2 font-mono" rows={12} value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} required />
